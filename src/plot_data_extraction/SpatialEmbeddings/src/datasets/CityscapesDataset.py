...
class CityscapesDataset(Dataset):
    ...
    def __init__(self, root_dir='./', type="train", class_id=26, size=None, transform=None):
        ...
        image_list += glob.glob(os.path.join(root_dir, 'leftImg8bit/{}/'.format(type), '*/*.jpg'))
        image_list.sort()
        self.image_list = image_list
        
        instance_list = []
        instance_list += glob.glob(os.path.join(root_dir, 'gtFine/{}/'.format(type), '*/*.png'))
        instance_list += glob.glob(os.path.join(root_dir, 'gtFine/{}/'.format(type), '*/*.jpg'))
...
#         print("current index: {}, total length: {}".format(index, len(self.instance_list)))
        sample = {}
